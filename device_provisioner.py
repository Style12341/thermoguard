...
def load_and_filter_data():
    ...
    try:
        ...
        required_cols = ['SERIAL_NUMBER', 'DEV_EUI', 'APP_KEY', 'VOLTAGE_MEASURED', 'VOLTAGE_REPORTED']
        ...
        df_clean = df.dropna(subset=['SERIAL_NUMBER', 'DEV_EUI'])
        
        if df_clean.empty:
            print("Error: No valid devices found (Check SERIAL_NUMBER and DEV_EUI columns).")
            return None
...
def main():
    ...
    if pd.isna(v_meas): v_meas = 1.0
    if pd.isna(v_rep): v_rep = 1.0
