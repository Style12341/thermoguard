...
#define CLK_PIN_1 PA5  // Example: Software SPI SCK for MAX6675
#define DATA_PIN_1 PA6 // Example: Software SPI MISO for MAX6675
#define CS_PIN_1 PA4   // Example: CS for MAX6675

// Battery Reading
#define VBAT_READ_PIN WB_A0 // Analog Input Pin
#define ADC_CTRL_PIN WB_IO4 // Pin to turn on voltage divider (if exists)
#define BATTERY_CHARGING_PIN PA0
...
void loop()
{
    ...
    // Fallback logic
    if (t0 == TEMP_ERROR_VALUE && lastValidTemperature[0] != TEMP_ERROR_VALUE)
        t0 = lastValidTemperature[0];
    if (t1 == TEMP_ERROR_VALUE && lastValidTemperature[1] != TEMP_ERROR_VALUE)
        t1 = lastValidTemperature[1];
    ...
    { // Valid temp found
        if (maxTemp <= TEMP_THRESHOLD_LOW)
            sleepTime = SLEEP_TIME_LOW_MS;
        else if (maxTemp >= TEMP_THRESHOLD_HIGH)
            sleepTime = SLEEP_TIME_HIGH_MS;
        else
        {
...
void readBatteryVoltage()
{
    ...
    if (batteryVoltage >= 4200)
        ...
        batteryPercentage = (uint8_t)((batteryVoltage - 3200) * 100 / (4200 - 3200));
